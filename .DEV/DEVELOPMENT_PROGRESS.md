...
## 2026-04-16：Variant-Effect Playbook 合同对齐与测试闭环

### 背景

基于本轮真实 `variant-effect` 执行反馈，目标是将 `playbooks/variant-effect`、`registry/output_contracts`、`docs/contracts`、评测样例与输入示例路径统一到当前有效入口，消除历史脚本路径残留造成的误导。

### 本轮改动

#### 1. Playbook 重构（通用执行模式）

- `playbooks/variant-effect/README.md`
  - 将原先以单位点 fast path 为主的结构升级为四层执行模式：
    1) `run_agent.sh` 编排；
    2) `execute_plan.sh` 验证/执行；
    3) AlphaGenome 单位点实跑；
    4) AlphaGenome VCF 批量实跑。
  - 新增网络超时处置说明（`dna_client.create` 超时后代理重试）。
  - 新增通用“结果验收模板”（文件存在、assembly 一致、status 分布、失败样本抽样）。

#### 2. Output Contract 对齐

- `registry/output_contracts.yaml`（`variant-effect`）
  - `runnable_steps` 移除旧注释入口：
    - `case-study/run_vcf_effect.py`
    - `case-study/run_variant_df_effect.py`
  - 替换为当前有效入口：
    - `skills/alphagenome-api/scripts/run_alphagenome_predict_variant.py`
    - `skills/alphagenome-api/scripts/run_alphagenome_vcf_batch.py`
  - `expected_outputs` 改为当前产物模式：
    - `alphagenome_variant-effect_<...>_result.json`
    - `alphagenome_variant-effect_<...>_rnaseq_overlay.png`
    - `<vcf_stem>_tissues.tsv`
  - `retry_policy` 与 recovery 语义对齐为：
    - `clarify-missing-inputs-then-connectivity-proxy-retry-once`

#### 3. 文档与样例引用清理

- `docs/contracts.md`
  - `variant-effect` 表格条目同步到最新合同值（assumptions / runnable_steps / expected_outputs / fallbacks / retry_policy）。
- `evals/task_success/cases.yaml`
  - `task_success_007` 注释与 query 更新为当前 VCF 路径表达；
  - 保持断言策略为通用校验，避免绑定单一 step 文本。
- `registry/input_schema.yaml`
  - `vcf-input.examples` 从旧固定路径改为通用示例：`path/to/variants.vcf`。
- `skills/alphagenome-api/scripts/run_alphagenome_vcf_batch.py`
  - `--input` 默认值更新为当前仓库有效样例路径：
    - `case-study-playbooks/variant-effect/vcf/Test.geuvadis.vcf`
- `case-study-playbooks/run_cases.sh`（本地 case-study 文案）
  - A1 描述改为当前路径和语义（该目录当前受 `.gitignore` 管理，不参与版本追踪）。
...
- `bash scripts/validate_task_success.sh` -> 通过（`19/19`）

#### C. 关键路径抽样

- `bash scripts/run_agent.sh --task variant-effect --query 'Use $alphagenome-api to run VCF batch variant effect prediction on path/to/variants.vcf, assembly hg38, output to output/alphagenome/.' --format json`
  - `plan.runnable_steps` 不再包含旧脚本名；
  - `expected_outputs` 与新合同一致；
...
- 全仓搜索确认已无以下主推荐路径残留：
  - `case-study/run_vcf_effect.py`
  - `case-study/run_variant_df_effect.py`
  - `case-study/Test.geuvadis.vcf`

### 影响与兼容性

- 未新增/删除 schema 字段，`run_agent.sh` 输出结构保持兼容；
- 变化聚焦于文档、合同文案与示例入口对齐，不改核心路由/执行逻辑。

## 2026-04-16：AlphaGenome Track-Prediction 端到端接入与真实测试闭环

### 背景

本轮目标：

1. 将 `alphagenome-api` 从“variant-effect 为主”升级为可直接参与 `track-prediction` 的常规技能。
2. 在 `run_agent` 中补齐 AlphaGenome 的 track 快路径（BED batch + 单区间）。
3. 完成注册表/文档/eval 联动与回归验证。

### 实施内容

#### 1. AlphaGenome 技能定义升级

- `skills/alphagenome-api/skill.yaml`
  - 新增 `track-prediction` task；
  - 增加 `predict_interval`、`track_prediction`、`rna_seq` 相关 trigger；
  - 增加 `species`、`output-head`、`sequence-or-interval` 输入映射；
  - 注册 `run_alphagenome_track_prediction_bed_batch.py` 脚本与 flags；
  - 约束文案扩展为兼容 variant + track 两类场景。
- `skills/alphagenome-api/SKILL.md`
  - 新增 `predict_interval` 真实 track workflow 章节；
  - 明确 BED 坐标约定（0-based half-open）与单区间输入；
  - 明确产物契约：`*_result.json` / `*_trackplot.png` / `*_track_prediction.npz` / batch summary。

#### 2. Track 脚本能力扩展（真实执行）

- `skills/alphagenome-api/scripts/run_alphagenome_track_prediction_bed_batch.py`
  - 输入模式扩展为 `--bed` 与 `--interval` 二选一；
  - 新增 `--species human|mouse` 并映射到 `dna_client.Organism`；
  - `predict_interval` 显式传入 organism；
  - 单区间与 BED 复用统一输出 schema；
  - 保留自动窗口扩展 + proxy retry 机制。

#### 3. 路由与注册表联动

- `registry/skills.yaml`：`alphagenome-api.tasks` 加入 `track-prediction`；
- `registry/tags.yaml`：`track-prediction` 候选加入 `alphagenome-api`；
- `registry/recovery_policies.yaml`：`track-prediction.fallback_skills` 加入 `alphagenome-api`；
- `scripts/run_agent.sh`：
  - 新增 `track-prediction + alphagenome-api` fastpath；
  - 支持 BED 与单区间两条真实 runnable step；
  - 默认值注入：`species=human`、`output-head=RNA_SEQ`、`ontology=UBERON:0001157`；
  - 加入 proxy fallback 与真实 expected outputs（summary/result/plot/npz/log）。
...
- 文档更新：
  ...
  - `skills/alphagenome-api/agents/openai.yaml`
- eval 增量：
  - `evals/task_success/cases.yaml` 新增
    - `task_success_018`（AlphaGenome BED batch）
    - `task_success_019`（AlphaGenome 单区间）

### 测试与验证结果

#### A. 真实 AlphaGenome track 执行

- 执行：`bash case-study-playbooks/track_prediction/run_alphagenome_track_case.sh`
- 数据：`case-study-playbooks/track_prediction/bed/Test.interval.bed`
- 结果：`4/4 success`（含首次超时后 proxy 重试成功）
- 产物目录：`case-study-playbooks/track_prediction/alphagenome_results/`
  - `alphagenome_track_bed_batch_summary.json`
  - 每区间 `*_result.json` / `*_trackplot.png` / `*_track_prediction.npz`

#### B. 结构与契约校验

- `bash scripts/validate_skill_metadata.sh --include-disabled` -> 通过
- `bash scripts/validate_input_contracts.sh --include-disabled` -> 通过

#### C. 路由与任务成功回归

- `bash scripts/validate_routing.sh` -> 通过（`21/21`）
- `bash scripts/validate_task_success.sh` -> 通过（`19/19`，含新增 018/019）

### 备注

- `run_agent` 现已可对 AlphaGenome track 需求生成可直接执行的计划（BED + interval）。
- 本轮未调整路由评分权重与 tie-break 逻辑，主要通过技能候选面与 fastpath 扩展实现“常规候选 + 可争主”。

## 2026-04-15：NTv3 教程同步、路由升级与 case-study 测试记录

### 背景

本轮目标：

1. 将 `Readme/NTv3` 教程更新同步到 stable 的 `nucleotide-transformer-v3` skill。
2. 将 NTv3 正式纳入 `fine-tuning` 路由，同时保持 `dnabert2` 在通用 CSV 场景下的默认优先级。
3. 执行 NTv3 embedding + fine-tuning 的最小 case-study 闭环并记录结果。

### 实施内容

#### 1. NTv3 技能内容同步

- 更新 `skills/nucleotide-transformer-v3/SKILL.md`：
  - 新增 notebook-first fine-tuning 分支（pretrained bigwig / posttrained bigwig / pretrained annotation）。
  - 增加训练前置检查项（token、依赖、长度整除、head 类型、硬件约束）。
  - 补充 fine-tuning 相关 grounded surface（DataLoader / AdamW / poisson_loss / focal_loss）。
- 新增参考文档：
  - `skills/nucleotide-transformer-v3/references/finetune-workflows.md`
  - 结构化沉淀 `Readme/NTv3/02/03/04` 三个训练教程。
- 更新 `skills/nucleotide-transformer-v3/references/setup-and-troubleshooting.md`：
  - 补充 fine-tuning 依赖与常见错误（`pyfaidx` / `pyBigWig` / `torchmetrics`、head/loss 对齐、中心裁剪一致性）。

#### 2. 路由与文档联动

- 更新 skill 元数据：
  - `skills/nucleotide-transformer-v3/skill.yaml`
  - 将 `fine-tuning` 纳入 tasks，补齐 fine-tuning 输入映射（`task-objective` / `dataset-schema` / `compute-constraints`）。
- 更新路由候选：
  - `registry/skills.yaml`
  - `registry/tags.yaml`
  - fine-tuning 候选顺序保持 `dnabert2` 在前，`nucleotide-transformer-v3` 在后。
- 更新策略与引导文档：
  - `agent/ROUTING.md`
  - `docs/routing.md`
  - `playbooks/fine-tuning/README.md`
  - `docs/skills-reference.md`
  - 明确规则：CSV/分类优先 `dnabert2`；bigwig/annotation/species-conditioned 目标优先 `nucleotide-transformer-v3`。

#### 3. 评测样例补充

- routing 新增 NTv3 fine-tuning 专项用例：
  - `evals/routing/cases.yaml` -> `route_019`
- groundedness 新增 NTv3 fine-tuning 专项用例：
  - `evals/groundedness/cases.yaml` -> `grounded_005`
- task-success 维持并验证 NTv3 相关 case：
  - `evals/task_success/cases.yaml` -> `task_success_012` / `task_success_013`
- 文档样例更新：
  - `docs/evals.md`
...
- `bash scripts/validate_routing.sh` -> 通过（19/19）
- `bash scripts/validate_groundedness.sh` -> 通过（5/5）
- `bash scripts/validate_task_success.sh` -> 通过（13/13）

#### NTv3 最小 case-study 执行

- `bash case-study/embedding/run_embedding_case.sh` -> 通过
  - 产物：
    - `case-study/embedding/output/ntv3_embedding_human_hg38_chr19_6700000_6702768_trackplot.png`
    - `case-study/embedding/output/ntv3_embedding_human_hg38_chr19_6700000_6702768_result.json`
- `bash case-study/fine-tuning/run_finetuning_case.sh` -> 通过
  - 产物：
    - `case-study/fine-tuning/output/fine_tuning_plan.json`
    - `case-study/fine-tuning/output/train-command.sh`
    - `case-study/fine-tuning/output/eval-metrics.json`
    - `case-study/fine-tuning/output/prep_report.json`
- `bash case-study/run_cases.sh --case A2` -> 通过
- `bash case-study/run_cases.sh --case A5` -> 通过
