...
- `scripts/run_agent.sh`: full orchestration output (routing/clarify + missing input checks + playbook mapping + structured plan)
- `scripts/execute_plan.sh`: dry-run/execute plan steps and verify expected outputs
...
- `registry/output_contracts.yaml`: normalized output-plan contracts for core tasks
- `registry/recovery_policies.yaml`: retry and fallback policy registry
